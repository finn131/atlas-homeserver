...
class AuthCorrelationRule(DetectionRule):
    ...
    def evaluate(self, conn, since_id) -> list:
        # Find SSH auth successes that are followed by nft_drop from same IP
        rows = conn.execute("""
            SELECT
                s.id as success_id,
                s.src_ip,
        ...
        for row in rows:
            ...
            # Evidence-based: medium severity, lower confidence
            # This is correlation evidence, NOT confirmed compromise
            detections.append(Detection(
                timestamp=datetime.now(timezone.utc),
                rule_name=self.name,
                severity=Severity.MEDIUM,
...
class DetectionEngine:
            ...
            with get_db(self.db_path) as conn:
                ...
                for rule in self.rules:
                    ...
                    for detection in detections:
                        ...
                        # Get hostname from first related event if available
                        hostname = ""
    ...
    def _handle_incident(self, conn, detection):
        ...
        Groups related detections by source IP and rule into incidents.
        """
        # Check for existing open incident from same src_ip + rule
